...
class RuleEvaluator:
    ...
    def evaluate(self, rules, graph, gnn_outputs=None):
        ...
        degrees = graph.in_degrees().float()    # [N] 节点度数
        ...
        for parsed in parsed_rules:
            result = self._evaluate_single_rule(parsed, ctx, labels)
            rule_results.append(result)

        # 所有规则组合评估 (OR 逻辑: 任意规则命中即标记)
        combined_result = self._evaluate_combined(parsed_rules, ctx, labels)
    ...
    def _evaluate_single_rule(self, parsed_rule, ctx, labels):
        """
        评估单条规则
    ...
    def _evaluate_combined(self, parsed_rules, ctx, labels):
        """评估所有规则的组合效果 (OR 逻辑)"""
